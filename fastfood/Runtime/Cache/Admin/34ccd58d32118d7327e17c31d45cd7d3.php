            ...
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
					...
					<form action="/bishe/CLY/index.php/Admin/Menu/index" method="get" class="form-inline">
					...
					<tbody>
					<?php if(is_array($list)): foreach($list as $key=>$me): ?><tr >
						  <td><?php echo ($me["id"]); ?></td>
						  <td><?php echo ($me["shopname"]); ?></td>
						  <td><?php echo ($me["menu"]); ?></td>
						  <td>￥<?php echo ($me["price"]); ?></td>
						  <td><img src="/bishe/CLY/Public/Uploads/s_<?php echo ($me["menu_pic"]); ?>"></td>
						  <td><?php echo ($me["num"]); ?> 份</td>
						  <td><?php echo ($me["label_id"]); ?></td>
						  <!--<td><?php echo ($me["area"]); ?></td>-->
						  

						  <td id="change<?php echo ($me["id"]); ?>"><?php if(($me["state"]) == "1"): ?>正在出售<?php else: ?>已下架<?php endif; ?></td>
						  <td><?php echo (date("Y-m-d",$me["addtime"])); ?></td>
						  <td id="button<?php echo ($me["id"]); ?>">
							<!--<button class="btn btn-xs btn-danger" onclick="off(<?php echo ($me["id"]); ?>)">下架</button><br><br>
							<button class="btn btn-xs btn-info" onclick="on(<?php echo ($me["id"]); ?>)">上架</button>-->
							<?php if($me["state"] == 1 ): ?><button onclick="off(<?php echo ($me["id"]); ?>,this);" class="btn btn-xs btn-danger" title="点击上架">下架</button>
							<?php else: ?>
								<button onclick="on(<?php echo ($me["id"]); ?>,this);" class="btn btn-xs btn-primary" title="点击下架">上架</button><?php endif; ?>
						  </td>
		...
		function off(id){
			Modal.confirm({msg:"确定下架吗？"}).on(function(e){
				if(e){
					$.ajax({
						type:'GET',             //请求方式GET
						url :'/bishe/CLY/index.php/Admin/Menu/off',           //ajax请求地址
						data:'id='+id, //请求传递的参数
		...
		function on(id){
			Modal.confirm({msg:"确定重新上架吗？"}).on(function(e){
				if(e){
					$.ajax({
						type:'GET',             //请求方式GET
						url :'/bishe/CLY/index.php/Admin/Menu/on',           //ajax请求地址
						data:'id='+id, //请求传递的参数
						dataType:'json',        //预期服务器返回的数据类型
						async : true,           //是否异步
						success: function(res){
