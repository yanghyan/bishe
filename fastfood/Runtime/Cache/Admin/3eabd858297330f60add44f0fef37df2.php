...
<html>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
                  <form  class="form-inline" action="/bishe/CLY/index.php/Admin/Shop/index" method="get">
                    ...
                    <select class="form-control" name="kind">
                      ...
                      <?php if(is_array($kind)): foreach($kind as $key=>$k): ?><option value="<?php echo ($k["id"]); ?>"><?php echo ($k["classname"]); ?></option><?php endforeach; endif; ?>
                    </select>
                  ...
                  <tbody>
                   <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <!-- <td></td> -->
                      <td><?php echo ($vo["k"]); ?></td>
                      <td><?php if(($vo["shoptype"] === '0') ): ?>企业商户<?php elseif(($vo["shoptype"] == 1)): ?>个体商户<?php endif; ?></td>
                     <!--  <td><img src="/bishe/CLY/Public/Uploads/S_<?php echo ($vo["env_pic"]); ?>"></td>
                      <td><img src=" /bishe/CLY/Public/Uploads/S_<?php echo ($vo["door_pic"]); ?>"></td>-->
                      <td><?php if(($vo["info"] === '0') ): ?>营业执照<?php elseif(($vo["info"] == 1)): ?>餐饮服务许可证<?php elseif(($vo["info"] == 2)): ?>其他资质<?php elseif(($vo["info"] == 3)): ?>无资质<?php endif; ?></td>
                      <!-- <td><img src="/bishe/CLY/Public/Uploads/S_<?php echo ($vo["info_pic"]); ?>"></td> -->
                      <td id="state<?php echo ($vo["id"]); ?>"><?php if(($vo["state"] === '0') ): ?>暂停营业<?php elseif(($vo["state"] == 1)): ?>审核中<?php elseif(($vo["state"] == 2)): ?>已审核<?php endif; ?></td>
                      <td><?php echo (date("Y-m-d",$vo["addtime"])); ?></td>
                      <td >
                        <button class="btn btn-xs btn-danger" id="myalert" onclick="doDel(this,<?php echo ($vo["id"]); ?>)">删除</button> 
                        <button class="btn btn-xs btn-primary" onclick="doEdit(<?php echo ($vo["id"]); ?>)">编辑</button> 
                        <span id="btn<?php echo ($vo["id"]); ?>">
                          <button class="btn btn-xs btn-success" onclick="doShen(<?php echo ($vo["id"]); ?>,<?php echo ($vo["state"]); ?>)">
                            <?php if(($vo["state"] === '0') ): ?>重新开业<?php elseif(($vo["state"] == 1)): ?>通过审核<?php elseif(($vo["state"] == 2)): ?>暂停营业<?php endif; ?>
                          </button>
                        </span> 
                      </td>
  ...
  <script type="text/javascript">
       ...
       function doDel(obj,id){
            Modal.confirm({msg:'确定要删除当前标签吗？'}).on(function(e){
                if (e) {
                    $.ajax({
                      type:"get",
                      url:"/bishe/CLY/index.php/Admin/Shop/del",
                      data:"id="+id,
                      dataType:"json",
      ...
      function doEdit(id) {
          window.location.href="/bishe/CLY/index.php/Admin/Shop/edit/id/"+id;
      }
      function doShen(id,state){

           $.ajax({
            type:"get",
            url:"/bishe/CLY/index.php/Admin/Shop/doShen",
