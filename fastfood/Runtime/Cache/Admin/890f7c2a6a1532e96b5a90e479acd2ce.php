      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li><a href="/bishe/CLY/index.php/Admin/Slide/index">轮播图信息</a></li>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
					<form class="form-inline" action="/bishe/CLY/index.php/Admin/Slide/index" method="get">
				...
				<br/>
                  <table id="tid"  width="300px" class="table table-bordered table-hover">
					...
					<thead>
					<tbody>
					   <?php if(is_array($slidelist)): foreach($slidelist as $key=>$slide): ?><tr class="slide<?php echo ($slide["id"]); ?>">
							  <td><?php echo ($slide["id"]); ?></td>
							  <td><img src="/bishe/CLY/Public/Admin/Uploads/s_<?php echo ($slide["pic"]); ?>" width="100px"/></td>
							  <td><?php echo ($slide["desc"]); ?></td>
							  <td><?php echo ($slide["link"]); ?></td>
							  <td>
								<?php if($slide["state"] == 0 ): ?><button onclick="doStart(<?php echo ($slide["id"]); ?>,this);" class="btn btn-xs btn-danger" title="点击启用">禁用</button>
								<?php else: ?>
									<button onclick="doStop(<?php echo ($slide["id"]); ?>,this);" class="btn btn-xs btn-primary" title="点击禁用">
										启用
									</botton><?php endif; ?>
							  </td>
							  <td><?php echo ($slide["addtime"]); ?></td>
							  <td>
								 <a href="javascript:;" onclick="doDel(<?php echo ($slide["id"]); ?>,this)" class="btn btn-xs btn-danger">删除</a> &nbsp;
								<a href="/bishe/CLY/index.php/Admin/Slide/edit/id/<?php echo ($slide["id"]); ?>" class="btn btn-xs btn-primary">编辑</a>
								&nbsp;
								</td>
                ...
                <div class="box-footer clearfix">
				  <a href="/bishe/CLY/index.php/Admin/Slide/add" class="btn btn-primary">添加轮播图</a>
	...
	<script type="text/javascript">
		function doDel(id,obj){
			if(confirm("确定要删除吗？")){
				$.ajax({
						type:'GET',
						url :'/bishe/CLY/index.php/Admin/Slide/del',
						data:'id='+id,
						dataType:'json',
		...
		function doStart(id,obj){
			if(confirm("确定启用吗？")){
				$.ajax({
					type:'GET',
					url :'/bishe/CLY/index.php/Admin/Slide/updateState',
					data:'id='+id,
					dataType:'json',
					success:function(res){
						if(res.b){
							$(obj).replaceWith("<button onclick='doStop(<?php echo ($slide["id"]); ?>,this);' class='btn btn-xs btn-primary' title='点击禁用'>启用</botton>");
						}else{
							alert(res.info);
						}
					}
		...
		function doStop(id,obj){
			if(confirm("确定禁用吗？")){
				$.ajax({
					type:'GET',
					url :'/bishe/CLY/index.php/Admin/Slide/updateState1',
					data:'id='+id,
					dataType:'json',
					success:function(res){
						if(res.b){
							$(obj).replaceWith("<button onclick='doStart(<?php echo ($slide["id"]); ?>,this);' class='btn btn-xs btn-danger' title='点击启用'>禁用</botton>");
						}else{
							alert(res.info);
						}
					}
