...
<html>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
                  <form  class="form-inline" action="/bishe/CLY/index.php/Admin/Users/index" method="get">
                  ...
                  <tbody>
                   <?php if(is_array($myuser)): $i = 0; $__LIST__ = $myuser;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr>
                      <td> <?php echo ($user["id"]); ?> </td>
                      <td><?php echo ($user["username"]); ?></td>
                      <td><?php echo ($user["name"]); ?></td>
                      <td><?php if(($user["sex"] == 1) ): ?>男<?php elseif(($user["sex"] === '0') ): ?>女<?php endif; ?></td>
                      <td><?php echo ($user["phone"]); ?></td>
                      <td><?php echo ($user["email"]); ?></td>
                      <td><?php if(($user["state"] == 1) ): ?>管理员<?php elseif(($user["state"] === '0')): ?>禁用<?php elseif(($user["state"] == 2)): ?>普通用户<?php elseif(($user["state"] == 3)): ?>商户<?php elseif(($user["state"] == 4)): ?>配送员<?php endif; ?> </td>
                      <td><?php echo (date("Y-m-d",$user["addtime"])); ?></td>
                      <td ><button class="btn btn-xs btn-danger" id="myalert" onclick="doDel(this,<?php echo ($user["id"]); ?>)">删除</button> 
                      <button class="btn btn-xs btn-primary" onclick="doEdit(<?php echo ($user["id"]); ?>)">编辑</button> <button class="btn btn-xs btn-warning" onclick="doPass(<?php echo ($user["id"]); ?>)">修改密码</button> </td>
  ...
  <script type="text/javascript">
       ...
       function doDel(obj,id){
            Modal.confirm({msg:'确定要删除当前标签吗？'}).on(function(e){
                if (e) {
                    $.ajax({
                      type:"get",
                      url:"/bishe/CLY/index.php/Admin/Users/del",
                      data:"id="+id,
                      dataType:"json",
      ...
      function doEdit(id) {
          window.location.href="/bishe/CLY/index.php/Admin/Users/edit/id/"+id;
      }
      function doPass(id){
          window.location.href="/bishe/CLY/index.php/Admin/Users/pass/id/"+id;
      }
  </script>
