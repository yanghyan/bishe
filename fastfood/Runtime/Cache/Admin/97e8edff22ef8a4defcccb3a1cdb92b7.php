          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
				<form class="form-inline" action="/bishe/CLY/index.php/Admin/Kind/index" method="get">
                  ...
                  <table id="tid"  width="300px" class="table table-bordered table-hover">
					...
					<thead>
					<tbody>
					   <?php if(is_array($kindlist)): foreach($kindlist as $key=>$kind): ?><tr class="kind<?php echo ($kind["id"]); ?>">
							  <td><?php echo ($kind["id"]); ?></td>
							  <td><?php echo ($kind["nbsp"]); ?>|---<?php echo ($kind["classname"]); ?></td>
							  <td><?php echo ($kind["pid"]); ?></td>
							  <td><?php echo ($kind["path"]); ?></td>
							  <td>
								 <button onclick="doDel(<?php echo ($kind["id"]); ?>,this)" class="btn btn-xs btn-danger">删除</button> &nbsp;
								<button onclick="doEdit(<?php echo ($kind["id"]); ?>)" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#updatekindModal">编辑</button>
								&nbsp;
								<button onclick="addc(<?php echo ($kind["id"]); ?>)" class="btn btn-xs btn-success" data-toggle="modal" data-target="#doAddchkindModal">添加子分类</button>
								</td>
   ...
   <script type="text/javascript">
	...
	<script type="text/javascript">
		...
		function doDel(id,obj){
			Modal.confirm({msg:'是否删除角色？'}).on(function (e) {
				if(e){
					$.ajax({
						type:'GET',
						url :'/bishe/CLY/index.php/Admin/Kind/del',
						data:'id='+id,
						dataType:'json',
		...
		function addc(id){
			$.ajax({
				type:'POST',
				url:'/bishe/CLY/index.php/Admin/Kind/addch',
				data:'id='+id,
				dataType:'json',
				success:function(res){
					$("#CPid").val(res.id);
		...
		function doAddch(){
			$.ajax({
				type:'POST',
				url: '/bishe/CLY/index.php/Admin/Kind/insert',
		...
		function doEdit(id){
			$.ajax({
				type:'POST',
				url:'/bishe/CLY/index.php/Admin/Kind/edit',
				data:'id='+id,
				dataType:'json',
				success:function(res){
					$("#editid").val(res.id);
		...
		function doUpdate(){
			$.ajax({
				type:'POST',
				url:'/bishe/CLY/index.php/Admin/Kind/update',
