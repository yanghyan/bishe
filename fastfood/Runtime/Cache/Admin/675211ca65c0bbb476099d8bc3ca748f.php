          ...
          <div class="row">
            <div class="col-md-12">
              <div class="box">
                ...
                <div class="box-body">
				<form class="form-inline" action="/bishe/CLY/index.php/Admin/Tag/index" method="get">
				...
				<br/>
                  <table id="tid"  width="300px" class="table table-bordered table-hover">
					...
					<thead>
					<tbody>
					   <?php if(is_array($taglist)): foreach($taglist as $key=>$tag): ?><tr class="tag<?php echo ($tag["id"]); ?>">
							  <td><?php echo ($tag["id"]); ?></td>
							  <td><?php echo ($tag["tag"]); ?></td>
							  <td><?php if(($tag["tag_kind"] == 1) ): ?>菜品标签<?php elseif(($tag["tag_kind"] == 2)): ?>位置区域<?php elseif(($tag["tag_kind"] == 3)): ?>价格区域<?php endif; ?></td>
							  <td><?php echo ($tag["addtime"]); ?></td>
							  <td>
								 <button onclick="doDel(<?php echo ($tag["id"]); ?>,this)" class="btn btn-xs btn-danger">删除</button> &nbsp;
								<button onclick="doEdit(<?php echo ($tag["id"]); ?>)" class="btn btn-xs btn-primary" data-toggle="modal" data-target="#updatetagModal">编辑</button>
								</td>
							  </td>
   ...
   <script type="text/javascript">
	...
	<script type="text/javascript">
		function doDel(id,obj){
			Modal.confirm({msg:'确定要删除当前标签吗？'}).on(function(e){
                if (e) {
					$.ajax({
						type:'GET',
						url :'/bishe/CLY/index.php/Admin/Tag/del',
						data:'id='+id,
						dataType:'json',
		...
		function doAdd(){
			$.ajax({
				type:'POST',
				url: '/bishe/CLY/index.php/Admin/Tag/insert',
		...
		function doEdit(id){
			$.ajax({
				type:'POST',
				url :'/bishe/CLY/index.php/Admin/Tag/edit',
				data:'id='+id,
				dataType:'json',
				success:function(res){
					$("#editid").val(res.id);
		...
		function doUpdate(){
			$.ajax({
				type:'POST',
				url :'/bishe/CLY/index.php/Admin/Tag/update',
