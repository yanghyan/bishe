...
<html>
			...
			<div class="body">
				<div class="wrap clearfix">
					...
					<div class="login-area" id="j-loginArea">
						<div class="title">登录美食美客</div>
							<div class="login" id="j-login">
							<form class="login_user" action="/bishe/CLY/index.php/Home/Public/doLogin/s/users/type/<?php echo ($type); ?>/id/<?php echo ($id); ?>" method="post"  onsubmit="return doSubmit()"  <?php if($info1!=''): ?>style="display:none"<?php else: ?>style="display:block"<?php endif; ?>>
								<p class="loginError">
									<span class="errorpic" <?php if($info!=''): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?>></span>
									<span class="errorinfo" id="info"  <?php if($info!=''): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?>><?php echo ($info); ?></span>
								</p>
								<p class="loginSMS">
									<a href="#">
										<span>短信快捷登录</span>
								...
								<p class="logincontent">
									...
									<span class="user">
										<input type="text" name="username" autocomplete="off" placeholder="手机/邮箱/用户名" style="background-color: #fff;"/>
									</span>
								</p>
								<p class="logincontent">
									<span><i class="fa fa-unlock-alt fa-2x"></i></span>
									<span class="pwd">
										<input type="password" name="pass" autocomplete="off" placeholder="密码" style="background-color: #fff;"/>
									</span>
								</p>
								<p class="loginpro">
									<span>
										<input  name="remberPass" class="pass-checkbox-input" checked="checked" type="checkbox">
										<label>
											下次自动登录
										</label>
									</span>
									<span>
										<a href="/bishe/CLY/index.php/Home/Public/forgetPW">
											登录遇到问题
										</a>
									</span>
								</p>
								...
								<p class="loginother">
									<a href="#">
										<img src="/bishe/CLY/Public/images/qq.jpg" title="qq登录" width="24px" height="24px"/>
									</a>
									<a href="#" class="weixin">
										<img src="/bishe/CLY/Public/images/weixin1.jpg" title="微信登录" height="24px"/>
									</a>
									<a href="#" class="weibo">
										<img src="/bishe/CLY/Public/images/weibo.jpg" title="新浪微博登录" height="24px"/>
									</a>
									<a href="#" class="renren">
										<img src="/bishe/CLY/Public/images/tengxun.jpg" title="腾讯微博登录" height="24px"/>
									</a>
								</p>
							</form>
							<!-- 手机号登录 -->
							<form class="login_phone" action="/bishe/CLY/index.php/Home/Public/doPLogin/type/<?php echo ($type); ?>" method="post"  onsubmit="return doPSubmit()" <?php if($info1!=''): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?>  >
							
								<p class="loginError">
									<span class="errorpic" <?php if($info1!=''): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?>></span>
									<span class="errorinfo" id="info1"  <?php if($info1!=''): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?>><?php echo ($info1); ?></span>
								</p>
								<p class="logincontent">
									<span><i class="fa  fa-mobile fa-2x"></i></span>
									<span class="phonenum">
										<input type="text" name="phone" class="pass-text-input pass-text-input-userName" autocomplete="off" placeholder="手机帐号" id="j_phone" />
									</span>
								</p>
								<p class="logincontent1">
									<span class="yzm">
										<input type="text" name="ddmm" class="pass-text-input pass-text-input-userName" autocomplete="off" placeholder="动态密码"/>
									</span>
								</p>
								<p class="logincontent2" onclick="sendPass()" id="send">
									<a href="#"  class="sendyzm" id="senda">
										发送动态密码
									</a>
								</p>
								<p class="loginsubmit">
	...
	<script type="text/javascript">
		...
		$(".logincontent .user input").focus(function(){
			$(this).parents("p").css("borderColor","#FF6699");
		}).blur(function(){
			$(this).parents("p").css("borderColor","#ddd");
		});
		
		$(".logincontent .pwd input").focus(function(){
			$(this).parents("p").css("borderColor","#FF6699");
		}).blur(function(){
			$(this).parents("p").css("borderColor","#ddd");
		});
		
		$(".logincontent .phonenum input").focus(function(){
			$(this).parents("p").css("borderColor","#FF6699");
		}).blur(function(){
			$(this).parents("p").css("borderColor","#ddd");
		});
		
		$(".logincontent1 .yzm input").focus(function(){
			$(this).parents("p").css("borderColor","#FF6699");
		}).blur(function(){
			$(this).parents("p").css("borderColor","#ddd");
		});
		...
		function checkUserName(){
			 ...
			 // else if(user.match(/^[a-zA-Z0-9_]{3,12}$/)==null){
			 // 	$(".loginError span").css("display","block");
    //            	$("#info").html("账号信息不正确！");
    //            	return false;
	   //       }
	    ...
	    function checkPass(){
	        var pass = $("input[name='pass']").val();
	         if(pass==""){
			 	...
			 	$("#info").html("请填写密码");
                return false;
			 }else if(pass.match(/^[a-zA-Z0-9_]{3,18}$/)==null){
			 	$(".loginError span").css("display","block");
	            $("#info").html("密码不正确");
	            return false;
	        }else{
