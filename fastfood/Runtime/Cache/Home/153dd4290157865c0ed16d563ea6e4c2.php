			...
			<div class="uc-main fr">
				<div class="w-list" id="J-orders-wrap">
					<ul class="w-status-tab clearfix" mon="area=filterStatus">
						<li class="current">
							<a href="javascript:;" class="first">收货地址</a>
						</li>
						<br>
						<a href="/bishe/CLY/index.php/Home/User/addAddr" class="link" id="">添加收货地址</a>
						<form class="layui-form" action="/bishe/CLY/index.php/Home/User/defaultAddr" method="post">
						<!--<input type="hidden" name="uid" value="<?php echo ($addr["uid"]); ?>"/>-->
							<table class="layui-table" lay-even="" lay-skin="row">
							  ...
							  <tbody>
							  <?php if(is_array($list)): foreach($list as $key=>$addr): ?><tr>
									  <td><?php echo ($addr["name"]); ?></td>
									  <td><?php echo ($addr["detail_add"]); ?></td>
									  <td><?php echo ($addr["phone"]); ?></td>
									  <td id="change<?php echo ($addr["id"]); ?>">
										<?php if($addr["state"] == 0): ?>默认地址
											<?php else: ?>普通地址<?php endif; ?>
									  </td>
									  <td>
										<?php if($addr["state"] == 1): ?><a onclick="doDefault(<?php echo ($addr["id"]); ?>,this);" class="link" id="">设为默认地址</a><br>
											<a onclick="doDel(<?php echo ($addr["id"]); ?>,this);" class="link" id="">删除</a> | 
											<a href="/bishe/CLY/index.php/Home/User/editAddr/id/<?php echo ($addr["id"]); ?>" class="link" id="">修改</a>
										<?php else: ?>
											<a onclick="doDel(<?php echo ($addr["id"]); ?>,this);" class="link" id="">删除</a> | 
											<a href="/bishe/CLY/index.php/Home/User/editAddr/id/<?php echo ($addr["id"]); ?>" class="link" id="">修改</a><?php endif; ?>
									  </td>
...
<script>
		...
		layui.use(['layer', 'form'], function(){
		  ...
		  layer.confirm('确定删除此地址吗？', {
			  ...
			  //执行Ajax发送并执行删除操作
				$.ajax({
					type:"GET",
					url: '/bishe/CLY/index.php/Home/User/delAddr',
					data:"id="+id,
					dataType:"json",
	...
	//设为默认地址
	function doDefault(id,bt){
		layui.use(['layer', 'form'], function(){
		  var layer = layui.layer;
		  layer.confirm('要将此地址设置为默认地址吗？', {
			  btn: ['确定','取消']
			}, function(){
			  //执行Ajax发送
				$.ajax({
					type:"GET",
					url: '/bishe/CLY/index.php/Home/User/defaultAddr',
					data:"id="+id,
					dataType:"json",
					success:function(res){
						if(res.b){
							//设为默认地址
							$("#change"+res.id).html("默认地址");
							$("#change"+res.oid).html("普通地址");
						}
					}
				});
			  layer.msg('完成了！', {icon: 1});
			});
		});
	}
