...
<div class="i_bg bg_color">
	...
	<div class="m_content">
		...
		<div class="m_right">
            ...
            <div class="mem_t">店铺信息</div>
            <table border="0" class="acc_tab" style="width:870px;" cellspacing="0" cellpadding="0">
              <tr>
                <td class="td_l">店铺名称： </td>
                <td><?php echo ($sh["shopname"]); ?></td>
              </tr>
              <tr>
                <td class="td_l b_none">店铺地址：</td>
                <td>
					<?php if(($sh["address"] == 1) ): ?>小店区
						<?php elseif(($sh["address"] == 2)): ?>迎泽区
						<?php elseif(($sh["address"] == 3)): ?>杏花岭区
						<?php elseif(($sh["address"] == 4)): ?>尖草坪区
						<?php elseif(($sh["address"] == 5)): ?>万柏林区
						<?php elseif(($sh["address"] == 6)): ?>晋源区<?php endif; ?>
				</td>
              </tr>
              <tr>
                <td class="td_l b_none">详细地址：</td>
                <td><?php echo ($sh["detail_add"]); ?></td>
              </tr>
              <tr>
                <td class="td_l">所属类别：</td>
                <td><?php echo ($sh["k"]); ?></td>
              </tr>
              <tr>
                <td class="td_l b_none">商户类型：</td>
                <td>
					<?php if(($sh["shoptype"] === '0') ): ?>企业商户
						<?php elseif(($sh["shoptype"] == 1)): ?>个体商户<?php endif; ?>
				</td>
              </tr>
              <tr>
                <td class="td_l">状态：</td>
                <td>
					<?php if(($sh["state"] === '0') ): ?>暂停营业
						<?php elseif(($sh["state"] == 1)): ?>审核中
						<?php elseif(($sh["state"] == 2)): ?>审核通过<?php endif; ?>
				</td>
              </tr>
              <tr>
                <td class="td_l b_none">注册时间：</td>
                <td><?php echo (date("Y-m-d",$sh["addtime"])); ?></td>
              </tr>
            </table>
