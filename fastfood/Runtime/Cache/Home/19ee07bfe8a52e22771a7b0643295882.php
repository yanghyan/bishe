        ...
        <table border="0" class="acc_tab" style="width:870px;" cellspacing="0" cellpadding="0">
              ...
              <tbody>
                <?php if(is_array($list)): foreach($list as $key=>$me): ?><tr >
                    ...
                    <td id="change<?php echo ($me["id"]); ?>"><?php if($me["state"] == 1 ): ?>在售<?php else: ?>已下架<?php endif; ?></td>
                    ...
                    <?php if($me["state"] == 1 ): ?><!-- <button onclick="edit()" class="btn btn-xs btn-danger" title="编辑">编辑</button>  -->
                                         
                      <button onclick="off(<?php echo ($me["id"]); ?>,this);" class="btn btn-xs btn-danger" title="点击上架">下架</button>
                    <?php else: ?>
   ...
   <script type="text/javascript">
      function doDel(obj,id){
            // Modal.confirm({msg:'确定要删除吗？'}).on(function(e){
                // if (e) {
                    $.ajax({
                      type:"get",
                      url:"/bishe/CLY/index.php/Home/Seller/delMenu",
                      data:"id="+id,
                      dataType:"json",
    ...
    function off(id){
      ...
      //   if(e){
          $.ajax({
            type:'GET',             //请求方式GET
            url :'/bishe/CLY/index.php/Home/Seller/off',           //ajax请求地址
            data:'id='+id, //请求传递的参数
    ...
    function on(id){
      ...
      //   if(e){
          $.ajax({
            type:'GET',             //请求方式GET
            url :'/bishe/CLY/index.php/Home/Seller/on',           //ajax请求地址
            data:'id='+id, //请求传递的参数
            dataType:'json',        //预期服务器返回的数据类型
            async : true,           //是否异步
            success: function(res){
