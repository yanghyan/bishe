...
<div class="header-bar static-hook-real static-hook-id-1" mon="area=navigation" id="header-bar">
			...
			<div class="uc-side fl">
				<ul class="w-nav clearfix" id="j-uc-nav" mon="element_type=nav" alog-alias="bainuo-user-pagetab" alog-group="bainuo-user-pagetab">
					<li class="nav-active last">
						<a class="nav-txt j-nav" href="javascript:;" onclick="doshow(1)">订单管理</a>
						<ul class="sub-nav" mon="area=subNav" id="u1">
							<li>
							<a href="/bishe/CLY/index.php/Home/Delivery/index" class="">
								<span class="sub-nav-ico">&#149;</span>已处理订单
							</a>
							</li>
							<li>
								<a href="/bishe/CLY/index.php/Home/Delivery/accept" class="">
									<span class="sub-nav-ico">&#149;</span>待处理订单
								</a>
							</li>
						</ul>
					</li>
			...
			<div class="uc-main fr">
				...
				<div class="w-list" id="J-orders-wrap">
					<ul class="w-status-tab clearfix" mon="area=filterStatus">
						<li class="current">
							<a href="javascript:;" class="first">已处理</a>
						...
						<table class="layui-table" lay-even="" lay-skin="row">
							  ...
							  <tbody>
							  <?php if(is_array($same)): foreach($same as $key=>$list): ?><tr>
									  ...
									  <td>
										...
										<?php else: echo (date("Y-m-d",$list["end_time"])); endif; ?>
									  </td>
									  <td id="change<?php echo ($list["id"]); ?>">
									  ...
									  <td>
										<?php if($list["state"] == 1): elseif($list["state"] == 2): ?>
										<?php elseif($list["state"] == 3): ?>
											<a onclick="doServerd(<?php echo ($list["id"]); ?>,this);" class="link" id="">送达</a>
										<?php else: ?>
											<a onclick="doDel(<?php echo ($list["id"]); ?>,this);" class="link" id="">删除订单</a><?php endif; ?>
									  </td>
	...
	<script>
		...
		//送达
		function doServerd(id,bt){
			layui.use(['layer', 'form'], function(){
			  var layer = layui.layer;
			  layer.confirm('确认送达订单吗？', {
				  btn: ['确定','取消']
				}, function(){
				  //执行Ajax发送并执行删除操作
					$.ajax({
						type:"GET",
						url: '/bishe/CLY/index.php/Home/Delivery/served',
						data:"id="+id,
						dataType:"json",
						success:function(res){
							if(res.b){
								$("#change"+res.id).html("配送完成");
							}
						}
					});
			...
			layui.use(['layer', 'form'], function(){
			  ...
			  layer.confirm('确认删除此订单吗？', {
				  ...
				  //执行Ajax发送并执行删除操作
					$.ajax({
						type:"GET",
						url: '/bishe/CLY/index.php/Home/Delivery/delOrder',
						data:"id="+id,
						dataType:"json",
						success:function(res){
							if(res.b){
