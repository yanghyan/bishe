        ...
        <table border="0" class="acc_tab" style="width:870px;" cellspacing="0" cellpadding="0">
              ...
              <tbody>
                <?php if(is_array($list)): foreach($list as $key=>$me): ?><tr>
                    ...
                    <td><?php echo ($me["order_num"]); ?></td>
                    <td><?php echo ($me["num"]); ?></td>
                    <td><?php echo (date("Y-m-d H:i:s",$me["start_time"])); ?></td>
                    <td><?php echo (date("Y-m-d H:i:s",$me["end_time"])); ?></td>
                    <!-- 1：已下单，2：已接单，3：配送中，4：配送完成 -->
                    <!-- <td><?php echo ($me["state"]); ?></td> -->
                    <td><?php if(($me["state"] == 1) ): ?>已下单<?php elseif(($me["state"] == 2) ): ?>已接单<?php elseif(($me["state"] == 3) ): ?>配送中<?php elseif(($me["state"] == 4) ): ?>已完成<?php endif; ?></td>
                    <td><?php echo ($me["remark"]); ?></td>                   
                   <!--  <td id="button<?php echo ($me["id"]); ?>">    
                   <!--  <button onclick="doDel(this,<?php echo ($me["id"]); ?>)" class="btn btn-xs btn-danger" title="删除">删除</button>  -->
            <!--                       <?php if(($me["state"] == 1) ): ?><button onclick="doJie(this,<?php echo ($me["id"]); ?>)" class="btn btn-xs btn-danger" title="删除">接单</button><button onclick="doJie(this,<?php echo ($me["id"]); ?>)" class="btn btn-xs btn-danger" title="删除">拒接</button> <?php elseif(($me["state"] == 2) ): ?>已接单<?php elseif(($me["state"] == 3) ): ?>配送中<?php elseif(($me["state"] == 4) ): ?><button onclick="doDel(this,<?php echo ($me["id"]); ?>)" class="btn btn-xs btn-danger" title="删除">删除</button><?php endif; ?>
